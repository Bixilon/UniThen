...
class JvmSqlHelper(file: File?) : SQLiteHelper {
    ...
    @Synchronized
    private fun createStatement(sql: String, vararg parameters: Any?, autoGeneratedKeys: Int? = null): PreparedStatement {
        val statement = autoGeneratedKeys?.let { connection.prepareStatement(sql, it) } ?: connection.prepareStatement(sql)
    ...
    override fun insert(sql: String, vararg parameters: Any?): Int {
        val statement = createStatement(sql, *parameters, autoGeneratedKeys = Statement.RETURN_GENERATED_KEYS)
