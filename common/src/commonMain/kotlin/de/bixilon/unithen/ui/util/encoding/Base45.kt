...
package de.bixilon.unithen.ui.util.encoding
...
// thanks https://github.com/ErikHellman/base45-kotlin/blob/main/src/main/kotlin/se/hellsoft/base45/Base45.kt

fun ByteArray.encodeBase45(): String {
    val lastGroupSize = size % 2
    val length = size / 2 * 3 + if (lastGroupSize != 0) 2 else 0
    val out = CharArray(length)
    val end = size - lastGroupSize
    var index = 0
    var i = 0
...
fun String.decodeAsBase45(): ByteArray {
    ...
    val tripleGroupSize = length - remainder
    val outputSize = (length / 3) * 2 + if (remainder == 0) 0 else 1
    val output = ByteArray(outputSize)
    var i = 0
    var index = 0

    while (i < tripleGroupSize) {
        val c = this[i + 0].code - MIN_CHAR_VALUE
        val d = this[i + 1].code - MIN_CHAR_VALUE
        val e = this[i + 2].code - MIN_CHAR_VALUE
